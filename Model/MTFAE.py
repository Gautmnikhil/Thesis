...
class FreEnc(nn.Module):
    def __init__(self, c_in, c_out, d_model, e_layers, win_size, fr):
        ...
        # Increase frequency retention to retain more features
        self.fr = fr if fr < 0.7 else 0.7  # Limit fr to a maximum of 0.7 to retain more features
...
class TemEnc(nn.Module):
    def __init__(self, c_in, c_out, d_model, e_layers, win_size, seq_size, tr):
        ...
        # Reduce temporal masking ratio to retain more temporal information
        self.tr = int(tr * win_size) if tr < 0.5 else int(0.45 * win_size)
    ...
    def forward(self, x):
        ...
        score = ltrd.sum(-1) / (ltrm.sum(-1) + 1e-8)  # Adding a small value to avoid division by zero

        # mask time points
        topk_tr = min(self.tr, score.shape[1])  # Ensure `topk` does not exceed available elements
        masked_idx, unmasked_idx = score.topk(topk_tr, dim=1, sorted=False)[1], (-1 * score).topk(score.shape[1] - topk_tr, dim=1, sorted=False)[1]
