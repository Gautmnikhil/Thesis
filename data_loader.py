...
class ProcessedUserLoader(Dataset):
    ...
    def __len__(self):
        ...
        if self.mode == "train":
            return len(self.train) // self.step
        elif self.mode == 'val':
            return len(self.val) // self.step
        elif self.mode == 'test':
            return len(self.test) // self.step
        elif self.mode == 'thre':
            return len(self.thre) // self.step
        else:
            return 0
    ...
    def __getitem__(self, index):
        ...
        if len(data) < self.win_size:
            padding = np.zeros((self.win_size - len(data), data.shape[1]), dtype=np.float32)
